...
p <- baseball_league1 %>% 
  ...
  theme(panel.border=element_blank(),
        ...
        legend.position="none", axis.text.x = element_text(size = 30),
        axis.text.y = element_text(size = 20),
        axis.title.y = element_text(size=30,margin = margin(t = 0, r = 20, b = 0, l = 0)))+
  coord_capped_cart(bottom='both', left='both')+
  xlab("") + ylab("Batting average")

direct.label(p, list("first.points", cex=1.4))


#cheating version to display only the first column of points
